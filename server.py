...
# ── Load API keys from environment variables (Leapcell deployment) ────────────
# AssemblyAI
ASSEMBLYAI_KEYS = []
...
# Try loading from config.py if it exists (local dev)
try:
    from config import GEMINI_KEY as _cfg_gem
    ASSEMBLYAI_KEYS = ASSEMBLYAI_KEYS or getattr(__import__('config'), 'ASSEMBLYAI_KEYS', [])
    if _cfg_gem and _cfg_gem not in GEMINI_KEYS:
        GEMINI_KEYS.insert(0, _cfg_gem)
    GEMINI_KEY = GEMINI_KEYS[0] if GEMINI_KEYS else ""
except ImportError:
    pass
...
# ── Startup summary ───────────────────────────────────────────────────────────
if ASSEMBLYAI_KEYS:
    print(f"[*] Loaded {len(ASSEMBLYAI_KEYS)} AssemblyAI key(s)")
else:
    print("[!] WARNING: No AssemblyAI keys found! Set ASSEMBLYAI_KEY env var.")

if GEMINI_KEYS:
    print(f"[*] Loaded {len(GEMINI_KEYS)} Gemini key(s)")
else:
    print("[!] WARNING: No Gemini keys found! Set GEMINI_KEY env var.")
...
def run_assemblyai_whisper(audio_path, original_filename):
    """Transcription via AssemblyAI with word-level timestamps."""
    import time
...
def run_transcribe(audio_path, original_filename):
    """Transcription via AssemblyAI only."""
    if not ASSEMBLYAI_KEYS:
        return {"error": (
...
def call_gemini_annotate_full(payload):
    """Entry point for /annotate — chunks if needed, calls Gemini."""
    ...
    return _annotate_chunk(payload)
...
def _chunked_annotate(payload, chunk_size):
    ...
    for i in range(0, len(words), chunk_size):
        ...
        for attempt in range(1, MAX_CHUNK_RETRIES + 1):
            result = _annotate_chunk(chunk_payload)
            if result.get("status") == "ok":
                last_error = None
                break
...
def _annotate_chunk(payload):
    """Annotate a single chunk of words via Gemini."""
    ...
    if not GEMINI_KEYS:
        return {"error": "No Gemini keys configured. Set GEMINI_KEY env var in Leapcell."}

    print(f"[*] Annotating {len(words)} words via Gemini...")
    for attempt in range(len(GEMINI_KEYS)):
        ...
        try:
            ...
            if resp.status_code == 429:
                ...
                continue
            if resp.status_code != 200:
                print(f"[!] Gemini error {resp.status_code} — rotating")
                continue
    ...
    return {"error": "All Gemini keys failed or rate-limited. Check your GEMINI_KEY env vars."}
    ...
    for i, attempt in enumerate([try1, try2, try3, try4, try5], 1):
        try:
            ...
            if isinstance(parsed, list):
                ...
                return {"status": "ok", "result": _build_result_from_anns(parsed)}
...
if __name__ == '__main__':
    ...
    print(f"  Transcription: {'AssemblyAI x' + str(len(ASSEMBLYAI_KEYS)) if ASSEMBLYAI_KEYS else '❌ NO ASSEMBLYAI_KEY SET'}")
    print(f"  Annotation   : {'Gemini x' + str(len(GEMINI_KEYS)) if GEMINI_KEYS else '❌ NO GEMINI_KEY SET'}")
